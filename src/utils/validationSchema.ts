...
export const signUpSchema = z.object({
  ...
  name: z.string().min(1, { message: "「名前」は20文字以内で入力してください" }).max(20, { message: "「名前」は20文字以内で入力してください" }),
});
...
export const basicInfoEditSchema = z.object({
  name: z.string().min(1, { message: "「名前」は20文字以内で入力してください" }).max(20, { message: "「名前」は20文字以内で入力してください" }),
  id: z.string().min(1, { message: "「ユーザーネーム」は20文字以内で入力してください" }).max(20, { message: "「ユーザーネーム」は20文字以内で入力してください" }),
  description: z.string().min(0, { message: "「説明」は100文字以内で入力してください" }).max(500, { message: "「説明」は500文字以内で入力してください" }).optional().nullable(),
  links: z.object({
...
export const planCreateSchema = z.object({
  name: z.string().min(1, { message: "「プラン名」は100文字以内で入力してください" }).max(100, { message: "「プラン名」は100文字以内で入力してください" }),
  description: z.string().min(0, { message: "「概要」は100文字以内で入力してください" }).max(500, { message: "「説明」は500文字以内で入力してください" }).optional().nullable(),
  price: z.number().min(1, { message: "「月額料金」は1円以上で入力してください" }),
  welcome_message: z.string().min(0, { message: "「新規プラン加入者へのメッセージ」は100文字以内で入力してください" }).max(1000, { message: "「新規プラン加入者へのメッセージ」は500文字以内で入力してください" }).optional().nullable(),
