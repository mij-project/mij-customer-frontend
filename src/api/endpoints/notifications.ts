...
export const getNotifications = async (type: 'system' | 'users' | 'payments', page: number, perPage: number = 20) => {
...
export const readNotification = async (type: 'system' | 'users' | 'payments', notificationId: string, userId: string) => {
  const request = {
    type: TYPE_NOTIFICATION_SYSTEM[type],
    user_id: userId,
