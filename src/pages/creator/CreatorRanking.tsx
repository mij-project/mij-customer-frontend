...
export default function CreatorRanking() {
  ...
  const [currentRankingCreatorsOverall, setCurrentRankingCreatorsOverall] = useState<
    RankingCreator[] | []
  >([]);
  const [currentRankingCreatorsCategories, setCurrentRankingCreatorsCategories] = useState<
    RankingCreatorCategories[] | []
  >([]);
    ...
    try {
      ...
      if (isFollowing) {
        setRankingCreatorsOverall((prev) => ({
          ...prev,
          ['all_time']:
            prev?.['all_time']?.length > 0
              ? prev?.['all_time'].map((creator) =>
          ...
          ['weekly']:
            prev?.['weekly']?.length > 0
              ...
              : [],
          ['monthly']:
            prev?.['monthly']?.length > 0
              ? prev?.['monthly'].map((creator) =>
                  creator.id === creatorId
                    ? {
                        ...creator,
                        follower_ids: creator.follower_ids.filter((id) => id !== user.id),
        ...
        {user ? (
          <>
            <CreatorsSection
              ...
              isSpecialShow={true}
            />
            {currentRankingCreatorsCategories &&
              currentRankingCreatorsCategories.map((category) => (
                <CreatorsSection
