...
export default function PostPriceTimesaleSettingEdit() {
  ...
  const [timeSaleStatus, setTimeSaleStatus] = useState<{
    is_active: boolean;
    is_expired: boolean;
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        // タイムセールIDから投稿詳細とタイムセール情報を一度に取得
        const initData = await getPostPriceTimeSaleEditByTimeSaleId(time_sale_id);

        // 投稿情報を設定
        if (initData.plan) {
          setPost(initData.plan);
        }
        ...
        if (initData.time_sale) {
          ...
          // タイムセール状態を設定
          setTimeSaleStatus({
            is_active: initData.time_sale.is_active,
            is_expired: initData.time_sale.is_expired,
            purchase_count: initData.time_sale.purchase_count,
            max_purchase_count: initData.time_sale.max_purchase_count || undefined,
          });
        } else {
          setFormError(['タイムセール情報が見つかりません。']);
        }
      } catch (e) {
        setFormError(['投稿またはタイムセール情報の取得に失敗しました。再度お試しください。']);
        console.error('Failed to fetch post or time sale:', e);
      } finally {
        setLoading(false);
      }
    };
