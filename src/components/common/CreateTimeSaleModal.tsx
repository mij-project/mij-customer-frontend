...
export default function CreateTimeSaleModal({ isOpen, onClose, onSubmit, originalPrice }: Props) {
  ...
  const validate = () => {
    ...
    if (useStartEnd) {
      ...
      if (startDateTime < new Date()) errors.push('開始日時は現在より後にしてください。');
      if (endDateTime < new Date()) errors.push('終了日時は現在より前にしてください。');
    }
                ...
                <div className="mt-4 p-3 rounded-md bg-white border border-blue-200">
                  ...
                  <div className="flex items-baseline gap-3 mt-2">
                    ...
                    <span className="text-lg font-bold text-blue-600">
                      ¥
                      {(originalPrice - Math.ceil(percent * originalPrice * 0.01)).toLocaleString()}
                    </span>
                  </div>
                </div>
              )}
