...
export default function CreatorCard({ creator, showRank = true, showFollowButton = true, onCreatorClick, onFollowClick }: CreatorCardProps) {
    ...
    return (
        <div className="flex items-center gap-1 border-black/20 border-b-[0.5px]">
            ...
            {
                !isSelf && (
                    <Button variant="subscribe" size="sm" onClick={() => { onFollowClick(creator.is_following, creator.id) }} disabled={showFollowButton}>
                        {creator.is_following ? <UserCheck className="h-4 w-4" /> : <UserPlus className="h-4 w-4" />}
                    </Button>
                )
            }
        </div >
