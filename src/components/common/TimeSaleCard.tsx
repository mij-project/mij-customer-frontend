...
export default function TimeSaleCard({
  item,
  originalPrice,
}: {
  item: any;
  ...
  const periodLabel = start && end ? `${start.substring(0, 16)} 〜 ${end.substring(0, 16)}` : '-';
    ...
    <div className="bg-white border border-gray-200 shadow-sm p-5 flex-col items-start justify-start space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span
            className={`px-3 py-1 ${isActive ? 'bg-secondary text-secondary-foreground' : 'bg-gray-100 text-gray-500'} text-sm font-bold`}
          >
            {leftBadge1}
          </span>
          ...
          {isActive && !isExpired && (
            <span className="px-3 py-1 bg-red-100 text-red-700 text-sm font-bold">
              セール中
            </span>
          )}
        </div>
